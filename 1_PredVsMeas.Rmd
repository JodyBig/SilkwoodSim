...
Five treatments:
	- Plant crop (2014-2015):
		+ T1 = 194 kgN/ha
		+ T2 = 134
		+ T3 = 94
		+ T4 = 94 (33Urea/66CRF)
		+ T5 = 67
	- Ratoon crop (2015-2016):
		+ T1 = 190 kgN/ha
		+ T2 = 158
		+ T3 = 110
		+ T4 = 112 (53Urea/47CRF)
...
tmp <-
  ...
  select(treatCode, treatValue, ratoon_no, sumDenit, sumNinDD, sumNinRO, biomassN_kg_ha, nit_totDiff) %>%
...
#### Plot a partial water balance ----
sim %>%
  select(treatCode, treatValue, ratoon_no, sumDrain, sumRunoff, sumRain, sumES, sumEP) %>%
  gather(variable, value, sumDrain, sumRunoff, sumRain, sumES, sumEP) %>%
  ggplot(aes(as.numeric(treatValue), value, colour=variable))+
  geom_line()+
  geom_point()+
  facet_wrap(~ratoon_no, ncol=4)+
  scale_colour_gdocs()+
...
sim1 <-
  sim %>%
  select(treatCode, treatValue, ratoon_no, yield_t_ha, biomass_t_ha) %>%
  mutate(ratoon_no = as.character(ratoon_no)) %>%
...
obs1 <-
  ...
  mutate(variable = factor(variable, levels=c("TCH_t_ha", "DM_t_ha"), labels = c('yield_t_ha', 'biomass_t_ha'))) %>%
  mutate(dataSource = 'measured') %>%
  select(treatCode, treatValue, ratoon_no, dataSource, variable, value)
...
d %>%
  ...
  geom_point()+
  facet_grid(variable~ratoon_no, scale='free_y', switch='y')+
  xlab('Fertiliser N Treatment (kgN/ha)')+
...
d1tmp <-
  ...
  ungroup() %>%
  left_join(treatDesc) %>%
  filter(fertType == 'urea') %>%
  select(dataSource, ratoon_no, fertN_kg_ha, percOfMaxN) %>%
  spread(fertN_kg_ha, percOfMaxN)

d1tmp

# d1FT <- vanilla.table(d1tmp)
# 
# for(i in 3:length(colnames(d1tmp))){
#   d1FT <- setFlexTableBackgroundColors(d1FT, j = i, colors = ifelse(d1tmp[,i] > 95, "orange", "white"))
# }
# 
# cat(as.html(d1FT))
```
...
regrD  <-
  d %>%
  group_by(dataSource, treatCode, ratoon_no, variable) %>%
...
regrD_nested <-
  regrD %>%
  ungroup() %>%
  select(ratoon_no, variable, measured, simulated) %>%
  nest(-ratoon_no, -variable)
...
grfRegressParams <- function(varName){
  funcParam %>%
    unnest(formula) %>%
    select(ratoon_no, variable, term, estimate) %>%
    mutate(term = ifelse(term=='measured', 'slope','intercept')) %>%
    filter(variable == varName) %>%
    ggplot(aes(ratoon_no, estimate, fill=ratoon_no))+
    geom_hline(yintercept=0)+
    geom_col()+
    geom_text(aes(label=round(estimate,1)), size=3)+
...
gofVariables <-
  model_nested %>%
  select(ratoon_no, variable, gof) %>%
  unnest()

grfVarStat <- function(varName){
  gofVariables %>%
    filter(gofTest %in% c('NSE','r','R2','RMSE')) %>%
    filter(variable == varName) %>%
    ggplot()+
    geom_hline(yintercept=0)+
    geom_col(aes(ratoon_no, tmp, fill=ratoon_no), position='dodge')+
    geom_text(aes(ratoon_no, tmp, label=tmp), size=3)+
    facet_grid(gofTest~., scales='free_y')+
    scale_fill_gdocs()+
    theme_few()+
    theme(axis.title.y = element_blank())
}
...
grfXY <- function(varName){
  regrD %>%
    filter(variable == varName) %>%
    ggplot(aes(measured, simulated, colour=ratoon_no))+
    geom_point()+
...
```{r}
grfRegressParams('yield_t_ha')
grfVarStat('yield_t_ha')
...
grfRegressParams('biomass_t_ha')
grfVarStat('biomass_t_ha')
```

## Biomass-N

```{r eval=FALSE, include=FALSE}
grfRegressParams('biomassN_kg_ha')
grfVarStat('biomassN_kg_ha')
